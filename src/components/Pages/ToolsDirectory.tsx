...
import { Search, Sparkles, ArrowRight, Code, Calculator, Clock, CheckCircle, Gamepad2, PenTool, Database, ShieldCheck, HeartPulse, GraduationCap, VideoIcon, Grid, Type, Cpu, Wrench, Crown, AlertCircle } from 'lucide-react';
...
const ToolsDirectory = () => {
        ...
        const fetchDbTools = async () => {
            ...
            try {
                ...
                if (res.tools && Array.isArray(res.tools)) {
                    ...
                    const fetchedDbTools = res.tools.filter((t: DbTool) => t.isActive !== false);
                    setDbTools(fetchedDbTools);
                } else {
                    toast.error(res.error || "فشل تحميل الأدوات");
                }
            } catch (error) {
                console.error("Failed to fetch tools from DB:", error);
                toast.error("حدث خطأ أثناء تحميل الأدوات.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    useEffect(() => {
        if (allTools.length > 0) {
            ...
            setTimeout(() => setFeaturedTools(shuffledTools.slice(0, 8)), 0);
        }
    }, [allTools]);
