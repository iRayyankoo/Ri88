...
const AdminSuite = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            // We need to import getAdminTools here, or fetch similarly
            const { getAdminStats, getUsers, getAdminTools } = await import('@/app/actions/admin');

            const [statsData, usersData, toolsRes] = await Promise.all([
                getAdminStats(),
                getUsers(),
                getAdminTools()
            ]);
                            ...
                            <table className="w-full text-right">
                                ...
                                <tbody className="divide-y divide-white/5 text-sm">
                                    {isLoadingTools ? (
                                        ...
                                        <tr key={tool.id} className="hover:bg-white/[0.03] transition-colors group">
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-3">
                                            ...
                                            <td className="px-8 py-6">
                                                <span className="text-slate-400 text-xs font-mono bg-white/5 px-3 py-1.5 rounded-xl border border-white/5">{tool.category}</span>
                                            </td>
                                            ...
                                            <td className="px-8 py-6">
                                                {tool.isActive ? (
                                                    ...
                                                    <span className="text-slate-500 text-xs font-bold bg-white/5 px-3 py-1.5 rounded-xl border border-white/10 w-fit">مخفية 👁️‍🗨️</span>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-8 py-6 text-left relative">
                                                <button
                                                    ...
                                                    className="p-2 rounded-xl hover:bg-white/10 text-slate-400 transition-colors inline-block border border-transparent hover:border-white/10"
                                                >
                                                ...
                                                {activeToolDropdown === tool.id && (
                                                    <div className="absolute left-8 top-1/2 z-50 mt-1 w-48 bg-[#1A1A24] border border-white/10 rounded-2xl shadow-2xl py-2 overflow-hidden">
                                                        <div className="absolute inset-0 bg-black/40 pointer-events-none" />

                                                        {/* Actions */}
                                                        <button onClick={() => handleUpdateTool(tool, { isPremium: !tool.isPremium })} className="relative w-full text-right px-4 py-2 hover:bg-white/5 text-xs font-bold text-white transition-colors">
                                                            {tool.isPremium ? 'جعلها مجانية' : 'تحويل لـ PRO'}
                                                        </button>
                                                        <button onClick={() => handleUpdateTool(tool, { isFreeForLimitedTime: !tool.isFreeForLimitedTime })} className="relative w-full text-right px-4 py-2 hover:bg-white/5 text-xs font-bold text-white transition-colors">
                                                            {tool.isFreeForLimitedTime ? 'إلغاء المؤقت' : 'مجاني لفترة'}
                                                        </button>
                                                        <div className="h-px w-full bg-white/10 my-1 relative" />
                                                        <button onClick={() => handleUpdateTool(tool, { isActive: !tool.isActive })} className="relative w-full text-right px-4 py-2 hover:bg-white/5 text-xs font-bold text-white transition-colors">
                                                            {tool.isActive ? 'إخفاء الأداة' : 'إظهار الأداة'}
                                                        </button>
                                                        <button onClick={() => handleUpdateTool(tool, { isMaintenance: !tool.isMaintenance })} className="relative w-full text-right px-4 py-2 hover:bg-white/5 text-xs font-bold text-white transition-colors">
